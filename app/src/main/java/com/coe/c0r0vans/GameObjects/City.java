            ...
            findViewById(R.id.buy).setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (up!=null) {
                        ...
                        raceBonus = ((1f - raceBonus / 4) * (100f - Player.getPlayer().getTrade()) / 100f);
                        int upcost = (int) (up.getCost() * raceBonus);
                        confirmWindow.setText(String.format(getContext().getString(R.string.approve_upgrade), up.getName(), up.getLevel(), StringUtils.intToStr(upcost)));
                    } else confirmWindow.setText(String.format(getContext().getString(R.string.approve_unknown_up), upgradeName));
                    confirmWindow.setConfirmAction(new Runnable() {
                        @Override
                        public void run() {
                            serverConnect.getInstance().ExecCommand(buyAction,
